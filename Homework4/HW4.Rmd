...
'candisc','car','dummies','corrplot','knitr','ggplot2','cowplot') 
...
### e) Repita parte c y d asuientod $p_1= 0.05$ y $p_2= 0.95$ y con $c(1|2) = c(2|1)$. ¡Es razonable esta eleccion de aprioris? Explique
...
par(mfcol = c(2, 2))
for (k in 1:2) {
  l <- names(hemophilia)[k]
  #br0 <- seq(min(datos[, k]), max(datos[, k]), le = 11)
  x.9 <- seq(min(hemophilia[, k]), max(hemophilia[, k]), le = 50)
  for (i in 1:2) {
    i0 <- levels(hemophilia$gr)[i]
    x <- hemophilia[hemophilia$gr == i0, l]
    hist(x, proba = T, col = grey(0.8), main = paste("tipo", i0),
    xlab = l)
    lines(x.9, dnorm(x.9, mean(x), sd(x)), col = "red", lwd = 2)
  }
}
```
