...
```{r}
datos_tidy <- melt(hemophilia, value.name = "valor")
aggregate(formula = valor ~ gr + variable, data = datos_tidy,  FUN = function(x){shapiro.test(x)$p.value})
```
...
par(mfcol = c(2, 2))
for (k in 1:2) {
  j0 <- names(hemophilia)[k]
  #br0 <- seq(min(datos[, k]), max(datos[, k]), le = 11)
  x0 <- seq(min(hemophilia[, k]), max(hemophilia[, k]), le = 50)
  for (i in 1:2) {
    i0 <- levels(hemophilia$gr)[i]
    x <- hemophilia[hemophilia$gr == i0, j0]
...
```{r}
modelo_lda <- lda(formula = gr ~ AHFactivity + AHFantigen, data = hemophilia)
modelo_lda
```
...
```{r}
predicciones <- predict(object = modelo_lda, newdata = hemophilia[, c(1,2)], method = "predictive")
